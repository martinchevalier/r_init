...
.ref <- function(text, link_html = NULL, link_pdf = NULL){
  if(.html & !is.null(link_html)) out <- paste0("<a href=\"", link_html, "\">", text, "</a>")
  if(.html & is.null(link_html)) out <- text
  if(.pdf & !is.null(link_pdf)) out <- paste0("\\href{", link_pdf, "}{", text, "}")
  if(.pdf & is.null(link_pdf)) out <- text
  cat(out)
}
...
- 
```{r, results = "asis", echo = FALSE}
.ref(text = "Module 1 : Prise en main du logiciel", link_html = "prise-en-main-du-logiciel.html")
```

- 
```{r, results = "asis", echo = FALSE}
.ref(text = "Module 2 : Manipuler les outils fondamentaux du langage", link_html = "manipuler-les-elements-fondamentaux-du-langage.html")
```

- 
```{r, results = "asis", echo = FALSE}
.ref(text = "Module 3 : Travailler avec des données statistiques", link_html = "travailler-avec-des-donnees-statistiques.html")
```

- 
```{r, results = "asis", echo = FALSE}
.ref(text = "Données utilisées pour les cas pratiques (.zip, 3,6 Mo)", link_html = "donnees.zip", link_pdf = "http://r.slmc.fr/donnees.zip")
```
...
```{r, results="asis", echo=FALSE}
if(.html) cat("<a href=\"..\">Retour à la page d'accueil</a>")
if(.pdf) if(.sol) cat("\\soltrue") else cat("\\solfalse")
```
